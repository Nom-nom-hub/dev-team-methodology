...
This document summarizes the three major improvements made to the team methodology templates to improve
...
Previously, command templates didn't explicitly state what files must exist before running. An agent could
run `/team.tasks` without `plan.md` existing, leading to workflow failures.
...
- **After**: Agent validates prerequisites before starting, provides clear error messages with recovery
  steps
...
The implement.md template complained that tasks were vague ("Implement parser functionality") but the
tasks template didn't distinguish between different task types. All tasks used the same structure, which
doesn't match how research vs implementation vs testing vs deployment actually work.
...
Templates mixed `[FEATURE_NAME]`, `[BRANCH]`, `[FEATURE-NUMBER]-[FEATURE-NAME]`, and `[BRANCH]`
inconsistently. Agents and humans couldn't tell what each meant or which to use when.
...
1. **Debugging failures**: Use AGENT-CHECKLIST.md error codes
...
1. **Progress Tracking Template** - Structured implementation-log.md with metrics
1. **Cross-File Navigation** - Add "See Also" sections linking related sections
1. **Validation Checklists** - Automated checking of phase gates
1. **Metrics Dashboard** - Track team performance across features
...
**Version**: 1.0 | **Created**: 2025-12-14 | **Impact**: High (Prevents workflow failures, improves task
quality, clarifies agent requirements)
