...
The text the user typed after `/team.analyze` in the triggering message **is** the analysis focus or specific areas to examine. Assume you always have it available in this conversation even if `{ARGS}` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
2. **Perform cross-artifact consistency analysis**:
...
3. **Analyze consistency from each team persona perspective**:
...
4. **Check for gaps and inconsistencies**:
...
5. **Validate team methodology compliance**:
...
7. **Generate comprehensive analysis report** with findings organized by:
...
8. **Create action items** for addressing identified issues:
...
9. **Write the analysis report** to `specs/[BRANCH]/analysis-report.md` with detailed findings and recommendations.
...
2. Include consistency matrix comparing all artifacts
3. Provide team persona-specific consistency analysis
4. Identify all gaps and inconsistencies
5. Assess risks related to inconsistency
6. Create actionable recommendations
7. Validate team methodology compliance
8. Update agent context with analysis results
