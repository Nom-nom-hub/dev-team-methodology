...
The text the user typed after `/team.review` in the triggering message **is** the review description or validation focus. Assume you always have it available in this conversation even if `{ARGS}` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
2. **Load the existing specification** from the feature directory.

3. **Validate specification against each team persona perspective**:
...
4. **Generate team validation checklist**:
...
2. List specific sections that need attention or improvement
3. Suggest specific updates to address validation issues
4. Update the specification in place with team perspective validation notes
5. Create or update the team validation checklist
6. Update agent context files with validation results
