...
The text the user typed after `/team.clarify` in the triggering message **is** the clarification focus or specific areas of concern. Assume you always have it available in this conversation even if `{ARGS}` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
2. **Analyze the specification for ambiguous or unclear areas**:
...
3. **Generate structured clarification questions organized by team persona impact**:
...
4. **Prioritize questions by impact and risk**:
...
5. **Format questions as structured decision tables**:
...
6. **Document all clarification activities** in `specs/[BRANCH]/clarification-log.md`:
...
7. **Update the specification** based on clarifications received:
...
8. **Validate that clarifications support team methodology**:
...
2. Create or update `specs/[BRANCH]/clarification-log.md` with all questions and answers
3. Update the specification with resolved clarifications
4. Prioritize questions by risk and impact
5. Format questions as decision tables with implications
6. Validate that all relevant team personas have input on major decisions
7. Update agent context with clarification results
